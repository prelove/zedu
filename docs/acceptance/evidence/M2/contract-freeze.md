...
- 固定错误：`40101` 未认证或 token 失效、`40102` 登录失败、`40103` 锁定、`40301` 权限不足、`40401` 不存在、`40901` 数据冲突、`42201` 状态不允许、`50001` 服务器内部错误、`50002` 数据库或事务错误。

> 勘误（2026-07-16）：`50001` 与 `50002` 已由 PRD v3.1 §14.3 定义，但首版冻结表遗漏。本条仅补齐既有 PRD 错误码；数据库、事务、查询或提交失败统一使用 HTTP 500 / `50002`，其他非数据库内部失败使用 HTTP 500 / `50001`。不得暴露底层 SQLite 错误文本。
